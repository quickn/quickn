package cn.json.common;

import cn.json.action.Action;
import cn.json.annotation.Filter;
import cn.json.filter.ActionFilter;
import cn.json.guice.QuickjModule;
import cn.json.hibernate.HibernateTemplate;
import cn.json.plugin.Plugin;
import cn.json.test.mock.QuickjMockModule;
...
public abstract class AbstractApplication implements Application {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see cn.json.Application#handle(javax.servlet.http.HttpServletRequest,
...
//			ok=true;
	 ...
	 * (non-Javadoc)
	 * 
	 * @see cn.json.Application#getModels()
	 */
		...
		if (filter != null && filter.name() != null) {
			...
			for (String filterName : filterNames) {
				...
				try {
					...
					try {
						c = Class.forName("cn.json.filter." + s[0]);
